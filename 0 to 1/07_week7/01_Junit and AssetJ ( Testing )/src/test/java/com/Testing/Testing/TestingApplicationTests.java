...
//	@AfterEach  // run method before each test case
	...
	void testDivideTwoNumbers_WhenDenominatorIsZero_ThenArithmeticException() {  // test case must be void
		...
		Assertions.assertThatThrownBy(()-> divideTwoNumbers(a,b)).isInstanceOf(ArithmeticException.class);  // so it will run this test case if ir throw a arithmetic exception else will throw error if it is actually any other 
